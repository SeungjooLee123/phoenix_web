...
<c:if test="${no eq page.curPage }">
	<a class="page_on">${no }</a>
	</c:if>
	
	<!-- 현재 페이지가 아닌경우 -->
	<c:if test="${no ne page.curPage }">
	<a class="page_off" onclick="go_page(${no})">${no }</a> <!-- no - 현재 페이지값 -->
	</c:if>
	
</c:forEach>

	<c:if test="${page.curBlock lt page.totalBlock }"> <!-- list = <와 같은 의미 -->
...
<style>
	...
	.page_next, .page_last, .page_first, .page_prev {
		width: 30px; height: 32px; color: #666; border: 1px solid gray; text-indent: -999999999px;
	}
	.page_next{
