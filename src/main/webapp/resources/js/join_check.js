...
var join = {
	// tag 의 상태를 확인할 함수를 선언하고 받아오는 값을 tag 변수로 설정
	tag_status: function(tag) {
		var data = tag.val(); 		//tag 내 입력된 값을 data 변수로 할당
		tag = tag.attr('name');		//tag의 name 속성의 값을 확인 ex)id, pw, email, ~~~

		if (tag == 'id') return this.id_status(data); //data tag에서 보내온 값 
		//id칸의 입력된 값을 보냄
	...
	, pw: {
		invalid: { code: 'invalid', desc: '비밀번호는 영어 소문자, 대문자, 숫자를 모두 포함' }
...
var space = /\s/g; // 전체 문자 내 공백이 있는지 여부를 확인하는 방법